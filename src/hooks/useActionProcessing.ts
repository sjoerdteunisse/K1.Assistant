...
import { getEffectiveReasoningModel, useSettingsStore } from "../stores/settingsStore";
...
export function useActionProcessing({ onSuccess, onError }: UseActionProcessingOptions) {
  ...
  const runAction = useCallback(
    async (
      ...
      const reasoningProvider = useSettingsStore.getState().reasoningProvider;

      if (!modelId && !options.isCloudMode) {
        if (reasoningProvider === "local") {
          onError(t("notes.actions.errors.noLocalModel", { defaultValue: "No local model downloaded. Go to Settings → AI Models to download one." }));
        } else {
